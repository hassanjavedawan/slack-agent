...
function buildInteractivePrompt(ctx: PromptContext): string {
	const lines = [
		...
		"",
		"## Viktor Spaces Apps",
		"- When building Spaces apps, the template is React 19 + Vite (src/App.tsx, NOT app/page.tsx).",
		"- Each app includes `convex/viktorTools.ts` with a `callTool` action for calling Viktor tools.",
		"- ALWAYS use `ctx.runAction(api.viktorTools.callTool, { role, arguments })` for AI/search/image calls.",
		"- NEVER write custom fetch() calls to Viktor APIs. NEVER hardcode any Viktor URLs.",
		"- Read the `viktor_spaces_dev` skill for the full workflow.",
		"",
		"## Current Context",
